package com.agenceteste.emprestcar.old.rest.exceptions;
...
import com.agenceteste.emprestcar.old.usecase.exceptions.BusinessException;
...
public class RestExceptionHandler {
	...
	public ResponseEntity<StandardError> businessException(BusinessException e, HttpServletRequest request) {
		StandardError err = new StandardError(System.currentTimeMillis(), HttpStatus.BAD_REQUEST.value(), "Business", e.getMessage(), request.getRequestURI());
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(err);
	}
